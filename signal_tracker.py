...
def render_full_signal_by_id(signal_id: str) -> Optional[str]:
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        return _render_full_html_from_signal(data["signal"] if "signal" in data else data)
    except Exception:
        return None
...
class SignalTracker:
    ...
    def _prune_signals(max_files: int = 2000):
        files = sorted(
            [os.path.join(SIGNAL_DIR, f) for f in os.listdir(SIGNAL_DIR) if f.endswith(".json")],
