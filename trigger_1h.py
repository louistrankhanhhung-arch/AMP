...
def _common_vals(s: Dict) -> Tuple[float, float, float]:
...
def check_long_trigger(s1h: Dict) -> Dict[str, Any]:
    ...
    reclaim = (pb.get("to_ma_tag") in ("touch", "near_ema20")) and (close >= ema20) and (rsi14 > 50) and bool(vol.get("pullback_vol_healthy"))
    if reclaim:
        return {"ok": True, "type": "reclaim", "reasons": ["to_ma_tag_ok", "close>=ema20", "rsi>50", "pullback_vol_healthy"]}
    ...
    if retest:
        return {"ok": True, "type": "retest", "reasons": ["bullish_reject", "near_ema20", "pullback_vol_healthy"]}
...
def check_short_trigger(s1h: Dict) -> Dict[str, Any]:
    ...
    reclaim_down = (pb.get("to_ma_tag") in ("touch", "near_ema20")) and (close <= ema20) and (rsi14 < 50) and bool(vol.get("pullback_vol_healthy"))
    if reclaim_down:
        return {"ok": True, "type": "reclaim_down", "reasons": ["to_ma_tag_ok", "close<=ema20", "rsi<50", "pullback_vol_healthy"]}

    rej_candle = _candles_any(candles, ["bearish_engulfing", "pin_bar_bear"])
    retest = rej_candle and _near(close, ema20, atr, k=0.25) and bool(vol.get("pullback_vol_healthy"))
    if retest:
        return {"ok": True, "type": "retest_down", "reasons": ["bearish_reject", "near_ema20", "pullback_vol_healthy"]}
